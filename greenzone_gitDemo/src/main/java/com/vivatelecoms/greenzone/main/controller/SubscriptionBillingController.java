...
import com.vivatelecoms.greenzone.model.ToneInfo;
...
public class SubscriptionBillingController {
	...
	@ResponseBody
	public String getToneIdInforXML(@RequestParam("serviceId") String serviceId,
			@RequestParam("subServiceId") String subServiceId, @RequestParam("offer") String offer,
			@RequestParam("aparty") String aparty, @RequestParam("bparty") String bparty,
			@RequestParam("station") String station, HttpServletRequest req, HttpServletResponse res) {

		logger.info("subscriptionBillingReq|aparty=" + aparty + "|bparty=" + bparty + "|offer=" + offer + "|station="
				+ station + "|serviceId=" + serviceId + "|subServiceId=" + subServiceId);
		...
		String jsonBodyData = "{\r\n" + "  \"msisdn\": \"" + aparty + "\",\r\n" + "  \"offer\": \"" + offer + "\"\r\n"
				+ "}";
		logger.info("jsonBodyData1=" + jsonBodyData);
		...
		if (serviceId.compareToIgnoreCase("crbt") == 0) {
			...
			String jsonCrbtBodyData = "{\r\n" + " \"cpid\": \"CRBT\",\r\n" + " \"msisdn\": \"" + aparty + "\",\r\n"
					...
					+ " \"langid\" :\"en\",\r\n" + " \"interfacename\" :\"IVR\",\r\n" + " \"timestamp\" :\"" + strDate
					+ "\",\r\n" + " \"issubcharge\" :\"Y\",\r\n" + " \"toneid\" :\""
					+ env.getProperty("DEFAULT_TONE_ID") + "\",\r\n" + " \"tonetype\" :\"0\",\r\n"
					+ " \"tonetypeidx\" :\"1\",\r\n" + " \"tonename\" :\"mysong\",\r\n" + " \"precrbtflag\" :\"\",\r\n"
			...
			postClientRes = postClientCrbtService.sendPostClientCrbtReq(coreEngineSubBillingUrl, jsonCrbtBodyData,
					"subscribe");
		} else {
		...
		String subStatus = "Y";
		if (serviceId.compareToIgnoreCase("chat") == 0) {
			String updateSubProfile = ChatUtils.updateSubProfileQuery(env.getProperty("SQL28_UPDATE_CHAT_SUB_PROFILE"),
					aparty, bparty, subStatus, offer);
			try {
		...
		/** insert default song ***/
		if (serviceId.compareToIgnoreCase("crbt") == 0
				&& !(postClientRes.equalsIgnoreCase("A") || postClientRes.equalsIgnoreCase("F"))) {
			String toneType = "0";
			String toneTypeIdx = "1";
			String callingParty = "D";
			String status = "A";
			String insertToneInfoQuery = ChatUtils.insertToneInfoQuery(env.getProperty("SQL32_INSERT_TONE_PROV_INFO"),
					aparty, toneType, toneTypeIdx, callingParty, env.getProperty("DEFAULT_TONE_ID"), status, songName);
		...
		if (serviceId.compareToIgnoreCase("chat") == 0) {
			String insertResult="Y";
			String insertQuery = ChatUtils.getQuery(env.getProperty("SQL48_CHAT_PROFILE_INSERT"), aparty);
			logger.info("chat profile query=" + insertQuery);
			try {
				int insertQueryResult = jdbcTemplate.update(insertQuery);
				if (insertQueryResult <= 0) {
					logger.error("Failed to insert chat profile|query=" + insertQueryResult);
					insertResult="F";
				} else {

					logger.info("Successfully to insert chat profile |resultChangesRow=" + insertQueryResult);
				}
				
			} catch (Exception e) {
				//logger.error("Exception occurred|Query=" + insertQuery + "|SQL exception=" + e);
				//e.printStackTrace();
				insertResult="F";
			}
			if(insertResult.equalsIgnoreCase("F"))
			{
				String updateSubProfile=ChatUtils.updateSubProfileQuery(env.getProperty("SQL28_UPDATE_CHAT_SUB_PROFILE"),aparty,bparty,subStatus,offer);
				try
				{
					int updateSubProfileResult=jdbcTemplate.update(updateSubProfile);
					if(updateSubProfileResult<=0)

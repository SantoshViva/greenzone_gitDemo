...
//				}
			...
			}else
			{
				...
				logger.info("thirdPartyUrl="+env.getProperty("THIRD_PARTY_PROFILE_URL"));
				String offer = env.getProperty("OFFER");
				String jsonBodyData = "{\r\n" +
						"  \"msisdn\": \"" +aparty +"\",\r\n" +
						"  \"offer\": \"" +serviceId +"\"\r\n" +
						"}";
				logger.info("jsonBodyData22="+jsonBodyData);
				String authKey=env.getProperty("THIRD_PARTY_AUTH_KEY");
				PostClientServiceImpl postClientService = new PostClientServiceImpl();
				postClientRes=postClientService.sendPostClientReq(thirdPartyProfileUrl, jsonBodyData,"check-subscription",authKey);

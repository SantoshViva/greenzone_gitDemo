...
import com.vivatelecoms.greenzone.model.ProfileResponseDetails;
...
public class PostClientServiceImpl implements PostClientService{
	...
	@Override
	public String sendPostClientReq(String url,String bodyData, String reqType, String authKey ) {
		
		
		
		logger.info("POST Client URL="+url+"|reqType="+reqType+"|authKey="+authKey);
		...
		try {
			...
			if(response.getStatusCode() == HttpStatus.CREATED) {
				...
				logger.trace("response json="+json.toString());
...
//							int errorCode=json.getInt("errorCode");
						...
						case "check-subscription-status" :
							String dataValue= json.getString("data");
							logger.info("check subscription response|result="+result+"|data="+dataValue);
							if(dataValue.equalsIgnoreCase("ACTIVE")||dataValue.equalsIgnoreCase("GRACE"))
							{
								returnValue ="Y";
							}else if(dataValue.equalsIgnoreCase("FAILED")||dataValue.equalsIgnoreCase("INACTIVE"))
							{
								returnValue ="L";
							}else {
								returnValue ="N";
...
//							int errorCode=json.getInt("errorCode");
							...
							break;
						case "check-subscription-status":
							logger.info("check-subscription response|result="+result);
							returnValue = "N";
							break;
						case "unsubscribe":
							logger.info("unbsubscribe response");
							break;
						default:
							logger.error("default case");
							break;
...
//							
			...
			logger.info("response Content-Type="+headers.get("Content-Type"));
			logger.info("response Server="+headers.get("Server"));
			...
			logger.info("Reosource Access Exception="+ ex.getMessage());
			logger.info("Reosource Access Exception="+ ex.getCause());
		}catch(Exception ex) {
